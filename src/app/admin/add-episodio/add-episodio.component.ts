...
export class AddEpisodioComponent implements OnInit {
  ...
  public misHipodromos : any;

  public isProgramacion : boolean

  public EpisodioFilter : any;

  public episodiosFilterAll : any;

  public episodioSelected : any;
  ...
  filterByName(e){
    ...
    this.EpisodioFilter = this.EpisodioFilter.filter(item => item.nombre === e.detail.value)
  }

  openEpisodioProgramacion(e){

    console.log(e)

    if(e.detail.checked === true){
      this.app.getEpisodio().subscribe(item =>{
        this.EpisodioFilter = item
        this.episodiosFilterAll = item
  
        this.EpisodioFilter = this.EpisodioFilter.filter(item => item.propio === true)
        console.log('jalando')
      })
    }else{
      console.log('cerrando')
      this.episodiosFilterAll = null
      this.EpisodioFilter = null
    }
  ...
  filterByPais(e){
      ...
      this.EpisodioFilter = this.EpisodioFilter.filter(item => item.pais === e.detail.value)
  ...
  onUpload(e){
   ...
   async eliminarEditarProgramacion(item) {
    const actionSheet = await this.action.create({
      ...
      buttons: [{
        ...
        icon: 'trash',
        handler: () => {
          this.quitarDeProgramacion(item)
        }
      }   ,{
        text: 'Cancelar',
        icon: 'close',
   ...
   quitarDeProgramacion(item){
     this.db.collection('episodios').doc(item.id).update({
       semanaActiva : false
     })
   }

   ysiEsHipodromo(){
     if(this.categoria === 'Hipismo'){
       this.misHipodromos = this.misHipodromos.filter(item=> item.nombre === this.nombre)

       console.log(this.misHipodromos)

       this.episodioSelected = this.misHipodromos[0]

       this.capituloNum()
     }
   }

   episdioSelected(item){
     this.episodioSelected = item
     this.EpisodioFilter = this.EpisodioFilter.filter(ite => ite.id === item.id)
   }
  ...
  publicar(){
    ...
    let endHourCode = this.mesEstreno +'/'+this.diaEstreno+ '/'+ this.ano + ' '+ this.hora
    this.EpisodioLongDate = this.diaEstreno + ' de ' + this.FullMonth + ' de ' + this.ano;
    ...
    if(numeral(this.diaEstreno).value() >= 0 && numeral(this.diaEstreno).value()<10){
      this.FechaDate =this.ano+ this.monthNum + '0'+this.diaEstreno
    }else{
      this.FechaDate = this.ano+ this.monthNum+this.diaEstreno
    }
  ...
  publicarProgramacion(){
    ...
    if(numeral(this.diaEstreno).value() >= 0 && numeral(this.diaEstreno).value()<10){
      this.FechaDate = this.year+this.monthNum + '0'+this.diaEstreno
    }else{
      this.FechaDate = this.year+this.monthNum+this.diaEstreno
    }


    this.db.collection('episodios').add({
      nombre:this.nombre,
      capitulo:this.episodioSelected.capitulo,
      link:this.episodioSelected.link,
      categoria:this.categoria,
      horario:this.horario,
      descripcion:this.episodioSelected.descripcion,
      cover:this.episodioSelected.cover,
      shortDate:this.EpisodioShortDate,
      longDate:this.EpisodioLongDate,
      day:this.dayOfTheWeek,
      destacado:this.destacado,
  ...
  publicarProgramacionSiesHipodromo(){
    switch(this.mesEstreno) {
      case '1':
        this.FullMonth = "Enero";
        this.monthNum = '01'
    ...
    this.db.collection('episodios').add({
      nombre:this.nombre,
      capitulo:this.capituloNro,
      link:this.episodioSelected.youtube,
      categoria:this.categoria,
      horario:this.horario, 
