...
export class HipodromoadminComponent implements OnInit {
  ...
  addHipo(){
    ...
    countDown(){
      const topDate = new Date(this.theHipodromo.falta);
    
      const updateClock = (date) => {
        if(this.theHipodromo){
  ...
  addCarrera(){
    ...
    if(this.dateFalta){
      ...
      if(this.mes === 'Enero'){
        this.mesToUpdate = 'January'
      }
      if(this.mes === 'Febrero'){
        this.mesToUpdate = 'February'
      }
      if(this.mes === 'Marzo'){
        this.mesToUpdate = 'March'
      }
      if(this.mes === 'Abril'){
        this.mesToUpdate = 'April'
      }
      if(this.mes === 'Mayo'){
        this.mesToUpdate = 'May'
      }
      if(this.mes === 'Junio'){
        this.mesToUpdate = 'June'
      }
      if(this.mes === 'Julio'){
        this.mesToUpdate = 'July'
      }
      if(this.mes === 'Agosto'){
        this.mesToUpdate = 'August'
      }
      if(this.mes === 'Septiembre'){
        this.mesToUpdate = 'September'
      }
      if(this.mes === 'Octubre'){
        this.mesToUpdate = 'October'
      }
      if(this.mes === 'Noviembre'){
        this.mesToUpdate = 'November'
      }
      if(this.mes === 'Diciembre'){
        this.mesToUpdate = 'December'
      }
      this.db.collection('hipodromos').doc(this.theHipodromo.id).update({
        falta:`${this.mesToUpdate} ${this.dia}, ${year} 12:00 GMT-04:00`,
      })
    }
