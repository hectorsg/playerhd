...
export class EnvivoAdminComponent implements OnInit {
  ...
  public newCarreraSource : string;
  ...
  async addingNews(x){
   ...
   if(numeral(day).value() >= 0 && numeral(day).value()<10){
    this.FechaDateEpisodios =year+this.monthNumEpiso + '0'+day
  }else{
    this.FechaDateEpisodios = year+this.monthNumEpiso+day
  }
  ...
  updateEnVivo(){
    ...
    if(this.newUrl === null){
      ...
      this.db.collection('hipodromos').doc(this.hipodromoData.id).update({
        ...
        carreraSource : this.newCarreraSource
      }).then(()=>{
