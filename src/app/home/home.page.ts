...
import { PicksComponent } from '../modales/picks/picks.component';
import { RetroAdminComponent } from '../admin/retro-admin/retro-admin.component';
import { AllpicksComponent } from '../modales/allpicks/allpicks.component';
...
export class HomePage {
  ...
  public pronostiSalir : boolean = true;

  public myPaisSelect : string;

  public allMyRetros : any;

  public paisPronos : any;
 ...
 async setTvLink(link){
  ...
  this.appSer.salaALL().subscribe(item =>{
   ...
   // this.knowMyDates()
    ...
    this.gacetaTitle = 'Todos los Retrospectos'
    this.gacetaTitleIng = 'All Race Program'
...
if(this.pulsoRuedita === true){
  ...
  this.moreLarge = 'height:50%;'
   ...
   this.gettingHipodromos()
    ...
    this.app.getPaisName().subscribe(item => {
      ...
      this.allPaises = this.allPaises.sort(sortBy('spot'))

      const arr1 = this.allPaises;
      const arr2 = this.misHipodromosActivos;

      const filterByReference = (arr1, arr2) => {
        let res = [];
        this.paisPronos = arr1.filter(el => {
           return arr2.find(element => {
              return element.pais === el.name;
           });
        });
        return this.paisPronos;
     }
  ...
  getJugadaByCountry(item){
    this.app.getJugadaSponsorByPais(item.name).subscribe(item => { 
      this.jugadaSponsor = item;
      this.jugadaSponsor.sort(sortBy('spot'))
    })
  }

  backJugadas(){
    this.jugadaSponsor = null
  }

  getPronosticosByCountry(item){

    this.myPaisSelect = item
    let d = new Date();
    let n = d.getMonth()
    let day = d.getDate()
   /* this.app.getPronosticoByPais(item).subscribe(item =>{
      let ShortDate = day.toString() + ' de ' + this.FullMonth 
      let pronosticosArray:any = item
      pronosticosArray = pronosticosArray.filter(item => item.aproved === 'aprobado')
      this.pronostiHoy = pronosticosArray.filter(item=> item.date === ShortDate)
      ...
      console.log(this.pronostiViejos, ' sin funciona')

      console.log('aqui es que esta el peo')

      this.pronostiSalir = false
      this.pronostiViejos.sort(sortBy('-fecha'))
    })*/
  }

  askPronos(ite){
    this.app.getRetrosByPais(ite.name).subscribe(item => {
      this.allMyRetros = item

      console.log(this.allMyRetros)
    })
  }

  backRetros(){
    this.allMyRetros = null
  }
  ...
  changePronos(e){
    ...
    let n = d.getMonth()
    let day = d.getDate()
    console.log(this.pronoIndexIs)
  /*  this.app.getPronosticoByPais(this.myPaisSelect).subscribe(item=>{
      let ShortDate = day.toString() + ' de ' + this.FullMonth 
      let pronosticosArray:any = item
      pronosticosArray = pronosticosArray.filter(item => item.aproved === 'aprobado')
      this.pronostiHoy = pronosticosArray.filter(item=> item.date === ShortDate)
  ...
  backPronostico(){
    ...
    this.pronostiSalir = true;
    this.myPaisSelect = null
 ...
 async uitarMudo(){
  ...
  gettingHipodromos(){
    this.app.getHipodromo().subscribe(item =>{
      ...
      console.log('Hipodromos Activos', this.misHipodromosActivos)
...
async  openAddEpisodio(){
 ...
 async openAdminPronostico(){
  ...
  openLinkJuega(item){
    let link:any =  this.sanitaizer.bypassSecurityTrustResourceUrl(item.link)
  ...
  async GacetaFiltroAction(){
    const actionSheet = await this.action.create({
      ...
      buttons: [ ,{
        ...
        handler: () => {
          this.allMyRetros = null
        this.app.getRetros().subscribe(item =>{
          this.allMyRetros = item
          this.gacetaTitle = 'Todos los Retrospectos'
          this.gacetaTitleIng = 'All Race Program'
        })
  ...
  createButtonsGaceta(){
    ...
    for (let index of this.PaisName) {
      let button = {
        ...
        handler: () => {
          this.allMyRetros = null
          this.app.getRetrosByPais(index.name).subscribe(item =>{
            this.allMyRetros = item
            this.gacetaTitle = 'Retrospectos de ' + index.name
            this.gacetaTitleIng = index.name + ' Race Programs'
          })
          return true;
        }
      }
