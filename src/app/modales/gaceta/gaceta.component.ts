...
export class GacetaComponent implements OnInit {
  ...
  ionViewDidEnter(){
    ...
    let gmt = d.getTimezoneOffset();

    console.log(gmt, '  Esta es la prueba')
  ...
  abrir(item){
    if(item.onSystem === false){
      this.link =  this.sanitaizer.bypassSecurityTrustResourceUrl(item.link)
      this.hipodromos.lockSwipes(false);
      this.hipodromos.slideTo(1);
      this.hipodromos.lockSwipes(true);
    }else{
      this.InBrowser.create(item.link, '_system')
    }
