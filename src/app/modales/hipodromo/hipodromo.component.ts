...
export class HipodromoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.db.collection('canalconfig').doc('sitioweb').valueChanges().subscribe(item => {
      this.irAl = item;
    })
    ...
    this.app.getEpisodio().subscribe(item =>{
      let allEpisodios : any = item
       ...
       //Aqui va el filtro de Episodios Destacado
        let firstFilter : any  = allEpisodios.filter(item => item.nombre.includes(this.hip.nombre))
       this.repetecionH = firstFilter
...
async closeFullScreen(){
  ...
  async verPost(item){
   ...
   await this.sleep(200)
    ...
    let link:any =  this.sanitaizer.bypassSecurityTrustResourceUrl(this.data.videoOptimizado)
